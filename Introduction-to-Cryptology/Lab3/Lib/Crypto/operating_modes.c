...
void aes_raw_CBC_encrypt(buffer_t *encrypted, buffer_t *in, buffer_t *key, buffer_t *IV){
...
void aes_raw_CBC_decrypt(buffer_t *decrypted, buffer_t *in, buffer_t *key){
	...
	if(in->length % BLOCK_LENGTH != 0){
		perror("[aes_raw_CBC_decrypt] : The input is not a vid cipher text.\n");
		return;
	}
	// 1. Initialisation
	buffer_t to_Xor;
...
void aes_CBC_decrypt(buffer_t *decrypted, buffer_t *encrypted, buffer_t *key,
	...
	if(!buffer_equality(&hash_test, &hash)){
		perror("[aes_CBC_decrypt] ERROR : Incompatible hash.\n");
		return;
	}
