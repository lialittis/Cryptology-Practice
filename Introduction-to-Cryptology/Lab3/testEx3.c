...
void test_get_padding_position(){
	...
	for(int a = 1; a <= BLOCK_LENGTH; a++){
		...
		printf("Padding position for a = %d :\t %d", a, pad_position);
...
void test_prepare(){
	...
	buffer_init(&plain, 2 * BLOCK_LENGTH);
	buffer_init(&encrypted, 3 * BLOCK_LENGTH);
	buffer_init(&key, BLOCK_LENGTH);
	buffer_init(&IV, BLOCK_LENGTH);
	buffer_init(&corrupted, 3 * BLOCK_LENGTH);
	buffer_init(&decrypted, 2 * BLOCK_LENGTH);
	
	// 2. Fill in buffer
	buffer_random(&plain, 2 * BLOCK_LENGTH);
	buffer_random(&IV, BLOCK_LENGTH);
	...
	int success = 1;
	for(int known_positions = BLOCK_LENGTH + 1; known_positions < 2 * BLOCK_LENGTH;
		known_positions++){
		printf("--- Known = %d:\n", known_positions);
		buffer_clone(&corrupted, &encrypted);
		prepare(&corrupted, &encrypted, &plain, known_positions);
		printf("Corrupted = ");
...
void test_find_last_byte(){
	...
	for(int j = 0; j < BLOCK_LENGTH; j++){
		...
		aes_raw_CBC_encrypt(&encrypted, &padded, &key, &IV);
		int pad_position = get_padding_position(&encrypted, &key) + encrypted.length - BLOCK_LENGTH;
		printf("Padding position = %d.\n\n",
			   pad_position);
		prepare(&corrupted, &encrypted, &padded, 4 * BLOCK_LENGTH - j);
		...
		hack = find_last_byte(&corrupted, pad_position - 1, &key);
...
void test_full_attack(){
	...
	fflush(stdout);
	full_attack(&decrypted, &encrypted, &key);
