...
void test1(){
    ...
    RSA_generate_key_files(pub_str, sec_str,
			   nbits, sec, state);
    printf("Done\n");
...
void test3(){
    ...
    /* 3. Sign */
    RSA_sign_buffer(sgn, &msg, N, d);
    gmp_printf("Message : %s\n\nSignature : %#Zx\n\n", message, sgn);

    /* 4. Verify */
    printf("Verification:   ");
    if(RSA_verify_signature(sgn, &msg, N, e))
	printf("[OK]\n\n");
    else
	printf("[FAILED]\n\n");
...
void test7(){
    ...
    /* 5. Verifies */
    printf("---- Verifying\n\n");
    int i = dsa_verify_buffer(&msg, p, q, a, r, s, y);
    if(i)
	printf("[OK]\n\n");
    else
	printf("[FAILED]\n\n");
...
void test11(){
    ...
    /* 6. Attack */
    dsa_attack(x_attack, &msg1, &msg2, p, q, a,
	       r1, s1, r2, s2);
